...
if(tcode!=null  && tcode.length()>=1 ) 
{
    ...
    try
    {
        con=ETLConnection.getConnection();
             String sql = "SELECT distinct  p.\"MDM_Code\",p.\"Plant_Code\" FROM \"STG_SCR_MRP\" p LEFT JOIN \"STG_SCR_Costing\" s ON  s.\"MDM_Code\" = p.\"MDM_Code\" and s.\"Plant_Code\" = p.\"Plant_Code\"  WHERE p.\"Approval_Status\"='Approved' and  s.\"MDM_Code\" IS NULL and s.\"Plant_Code\" IS NULL and p.\"Plant_Code\"  IN ('1000','1010','1020','1030','1040','1050','1060','1070','1080','1090','1100','1800','1802','1803','2010','3010','3020','3030','3800')  and  p.\"MDM_Code\"=? UNION ALL SELECT distinct  p.\"MDM_Code\",p.\"Plant_Code\" FROM \"STG_SCR_Sales\" p LEFT JOIN \"STG_SCR_Costing\" s ON  s.\"MDM_Code\" = p.\"MDM_Code\" and s.\"Plant_Code\" = p.\"Plant_Code\"  WHERE p.\"Approval_Status\"='Approved' and  s.\"MDM_Code\" IS NULL and s.\"Plant_Code\" IS NULL and  p.\"MDM_Code\"=?   ";           
         pstmt=con.prepareStatement(sql); //sql select query      

       
         //pstmt=con.prepareStatement("select distinct  \"Plant_Code\"  from \"STG_SCR_Sales\" where \"Approval_Status\" ='Approved' and \"MDM_Code\"=?"); //sql select query      
         pstmt.setInt(1,id);
         pstmt.setInt(2,id);
