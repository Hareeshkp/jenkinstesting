...
public List<PlantCode> plantcodelist() throws SQLException, ClassNotFoundException
{
	Connection con=null;
    ...
    List<PlantCode> plantlist = new ArrayList<>();
     try
     {
    	...
    	ps=con.prepareStatement("select distinct \"Plant_Code\" from \"STG_SCR_Plant_Storage_Purchase\" order by \"Plant_Code\"");  
        rs=ps.executeQuery(); 
        while (rs.next()) 
        {
     ...
     ResultSet rs=null;
        ...
        List<PlantCode> plantlist = new ArrayList<>();
         try
         {
        	 ...
        	 {
        		 ps=con.prepareStatement("select distinct \"Plant_Code\" from \"STG_SCR_Purchase\" order by \"Plant_Code\"  ");  
		             rs=ps.executeQuery();  
        	 }
	           
        	 else
        	 {
        		 ps=con.prepareStatement("select distinct \"Plant_Code\" from \"STG_SCR_Purchase\" where \"Material_Type_Code\"=? order by \"Plant_Code\"  ");
