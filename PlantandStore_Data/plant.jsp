...
if(tcode!=null  && tcode.length()>=1 ) 
{
    ...
    try
    {
        con=ETLConnection.getConnection();
        	...
        	String sql = "SELECT distinct  p.\"MDM_Code\",p.\"Plant_Code\" FROM \"STG_SCR_Purchase\" p LEFT JOIN \"STG_SCR_Plant_Storage\" s ON  s.\"MDM_Code\" = p.\"MDM_Code\" and s.\"Plant_Code\" = p.\"Plant_Code\"  WHERE p.\"Approval_Status\"='Approved' and  s.\"MDM_Code\" IS NULL and s.\"Plant_Code\" IS NULL and  p.\"MDM_Code\"=?  ";           

      //  pstmt=con.prepareStatement("select  \"Plant_Code\"  from \"STG_SCR_Purchase\" where \"Approval_Status\" ='Approved' and \"MDM_Code\"=?"); //sql select query      
