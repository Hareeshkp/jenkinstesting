     ...
     ResultSet rs=null;
        ...
        List<PlantCode> plantlist = new ArrayList<>();
         try
         {
        	 ...
        	 {
        		 ps=con.prepareStatement("select distinct \"Plant_Code\" from \"STG_SCR_Plant_Storage\" order by \"Plant_Code\"  ");  
		             rs=ps.executeQuery();  
        	 }
	           
        	 else
        	 {
        		 ps=con.prepareStatement("select distinct \"Plant_Code\" from \"STG_SCR_Plant_Storage\" where \"Material_Type_Code\"=? order by \"Plant_Code\"  ");
