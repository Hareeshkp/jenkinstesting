...
@WebServlet("/ScrbasicPlantDraft")
public class ScrbasicPlantDraft extends HttpServlet 
{  
...
public void doPost(HttpServletRequest request, HttpServletResponse response)  
	...
	if (modifiedby != null ) 
	{
	    ...
	    if(coust==null || coust.length<1)
	    {
	    	 ...
	    	 RequestDispatcher rd=request.getRequestDispatcher("Basic_Plant_Mapping_View_Draft.jsp");  
	  	     rd.include(request, response); 
	    }
	    if(coust!=null || coust.length>=1)
	    {
	    	int  countrow=PlantMappingDAO.DeleteDraft(ttcode);
	    	if(countrow>=1)
	    	{
	    	    int statuss=PlantMappingDAO.UpdateDraft(coust,ttcode);
